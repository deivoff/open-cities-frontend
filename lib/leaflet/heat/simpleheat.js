  ...
  draw(minOpacity) {
    ...
    for (let i = 0, len = this._data.length, p; i < len; i++) {
      ...
      ctx.globalAlpha = Math.min(
        Math.max(p[2] / this._max, minOpacity === undefined ? 0.05 : minOpacity), 1,
      );
      ctx.drawImage(this._circle, p[0] - this._r, p[1] - this._r);
    }

    // colorize the heatmap, using opacity value
    // of each pixel to get the right color from our gradient
